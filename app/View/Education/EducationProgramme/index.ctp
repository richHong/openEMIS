...
$this->start('tabActions');
	...
	if(count($data) > 1) {
		echo $this->FormUtility->link('reorder', array('controller' => $this->params['controller'], 'action' => 'edit', $model));
	}
$this->end();
		...
		<table class="table table-hover table-striped table-bordered table-highlight">
			<thead>
				<tr>
					...
					<th><?php echo $this->Label->get('general.duration'); ?></th>
			...
			<tbody>
				...
				<tr row-id="<?php echo $obj[$model]['id']; ?>">
					...
					<td><?php echo $obj[$model]['duration']; ?></td>
