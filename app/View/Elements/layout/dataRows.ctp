...
<div class="form-group">
	<label class="<?php echo $defaults['label']['class'] ?>"><?php echo $dataRowName;//echo $this->Label->get('CensusGrid.x_categories') ?></label>
	<div class="col-md-6">
		<div class="table-responsive">
			<table class="table table-striped table-hover table-bordered">
				...
				<tbody>
				<?php 
				if (($action == 'add' || $action == 'edit') && isset($this->data[$dataRowName])) :
					foreach ($this->data[$dataRowName] as $i => $obj) :
				?>
					<tr>
						<td>
							<?php
			...
			<a class="void icon_plus" onclick="$('#<?php echo $dataRowName; ?>').click()"><?php echo $this->Label->get('general.add') ?></a>
		</div>
	</div>
</div>
